...
fn main() {
    ...
    let lib_dir = if cfg!(target_os = "macos") {
        ...
        let lib_dir = out_dir.join(NUGET_PACKAGE_ID).join("build/native/lib");
        if !lib_dir.exists() {
            let status = Command::new("nuget.exe")
                .args([
                    "install",
                    NUGET_PACKAGE_ID,
                    "-OutputDirectory",
            ...
            fs::rename(lib_dir.join("../win-x64"), &lib_dir).unwrap();
        }
        lib_dir
    } else {
        let lib_dir = out_dir.join("mkl/latest/lib");
        if !lib_dir.exists() {
            let res = ureq::get(LINUX_INSTALLER_URL)
                .call()
            ...
            let installer_name = LINUX_INSTALLER_URL.split('/').last().unwrap();
            io::copy(
